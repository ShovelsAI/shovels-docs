  ...
  <Tabs>
    ...
    <Tab title="EDL (Enterprise Data License)">
      ...
      * Added `first_seen_date` to the `permits` table
      ...
      <Info>This is different than the `first_seen_date` column in the `contractors` table, which by contrast represents the date the contractor was first seen in our platform and assigned an `id`.</Info>
    </Tab>
  </Tabs>
</Update>

<Update label="V2.0.3" description="2025-02-06">
  <Warning>FINAL WARNING: As previously announced, the V1 of the Shovels API is **now deprecated**.</Warning>
